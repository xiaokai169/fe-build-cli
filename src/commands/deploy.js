...
export async function deployCommand(config) {
  ...
  const transferMode = transferIndex !== -1 && args[transferIndex + 1]
    ...
    : undefined;
  if (transferMode && !['pipe', 'rsync', 'sftp', 'obs'].includes(transferMode)) {
    console.error(`⚠️  无效的传输模式: ${transferMode}，支持: pipe, rsync, sftp, obs`);
  ...
  for (let i = 0; i < selectedServers.length; i++) {
    ...
    try {
      await deployToServer({
        ...
        transferMode: ['pipe', 'rsync', 'sftp', 'obs'].includes(transferMode) ? transferMode : undefined
      });
